...
namespace HLStrafe
{
		...
		switch (curState.Parameters.Type) {
			case HLTAS::ConstraintsType::VELOCITY:
			{
				if (curState.Parameters.Parameters.Velocity.Constraints == 0)
					return std::optional(static_cast<uint16_t>((vel_yaw + curState.TargetYawOffset * M_DEG2RAD) * M_INVU_RAD));
			} break;
	...
	static inline VCT::AngleConstraints ComputeConstraints(double vel_yaw, const CurrentState& curState)
	{
		...
		switch (curState.Parameters.Type) {
			case HLTAS::ConstraintsType::VELOCITY:
			{
				...
				return VCT::AngleConstraints(
					static_cast<int>((vel_yaw + 
						(curState.TargetYawOffset - curState.Parameters.Parameters.Velocity.Constraints) * M_DEG2RAD) * M_INVU_RAD),
					static_cast<int>(std::ceil((vel_yaw + 
						(curState.TargetYawOffset + curState.Parameters.Parameters.Velocity.Constraints) * M_DEG2RAD) * M_INVU_RAD))
				);
			} break;
			...
			case HLTAS::ConstraintsType::VELOCITY_AVG:
			{
				...
				return VCT::AngleConstraints(
					static_cast<int>((vel_yaw + 
						(curState.TargetYawOffset - curState.Parameters.Parameters.VelocityAvg.Constraints) * M_DEG2RAD) * M_INVU_RAD),
					static_cast<int>(std::ceil((vel_yaw + 
						(curState.TargetYawOffset + curState.Parameters.Parameters.VelocityAvg.Constraints) * M_DEG2RAD) * M_INVU_RAD))
				);
			} break;
			...
			case HLTAS::ConstraintsType::VELOCITY_LOCK:
			{
				...
				return VCT::AngleConstraints(
					static_cast<int>((vel_yaw + 
						(curState.TargetYawOffset - curState.Parameters.Parameters.VelocityLock.Constraints) * M_DEG2RAD) * M_INVU_RAD),
					static_cast<int>(std::ceil((vel_yaw + 
						(curState.TargetYawOffset + curState.Parameters.Parameters.VelocityLock.Constraints) * M_DEG2RAD) * M_INVU_RAD))
				);
			} break;
		...
		if (out.Duck) {
			...
			if (player.InDuckAnimation
				...
				if (postype == PositionType::GROUND) {
					...
					return GetPositionType(player, traceFunc, pointContentsFunc, version);
				}
			}
		} else {
			// Unduck.
	...
	PositionType PredictJump(PlayerData& player, PositionType postype, const MovementVars& vars, const HLTAS::Frame& frame, CurrentState& curState, ProcessedFrame& out, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version, bool decreaseDwjTimes)
	{
		assert(postype != PositionType::WATER);
		...
		if ((frame.Dwj || curState.DwjsLeft)
			...
			out.Duck = true;
			postype = PredictDuck(player, vars, postype, curState, out, traceFunc, pointContentsFunc, version);
			if (postype != PositionType::GROUND) {
				out = out_backup;
	...
	void Jumpbug(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		if (player.Ducking) {
			outCopy.Duck = false;
			postype = PredictDuck(playerCopy, vars, postype, curStateCopy, outCopy, traceFunc, pointContentsFunc, version);
	...
	void Dbc(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		auto curStateCopy = CurrentState(curState);
		postype = PredictDuck(playerCopy, vars, postype, curStateCopy, out, traceFunc, pointContentsFunc, version);
		...
		postype = PredictDuck(playerCopy, vars, postype, curStateCopy, out_temp, traceFunc, pointContentsFunc, version);
	...
	void LgagstDucktap(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		postype = PredictDuck(air, vars, postype, curStateCopy2, out_temp, traceFunc, pointContentsFunc, version);
	...
	void LgagstJump(const PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		out_temp.Jump = true;
		curStateCopy = CurrentState(curState);
		postype = PredictJump(air, postype, vars, frame, curStateCopy, out_temp, traceFunc, pointContentsFunc, version);
	...
	PositionType Strafe(PlayerData& player, const MovementVars& vars, PositionType postype, const HLTAS::Frame& frame, ProcessedFrame& out, bool reduceWishspeed, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, bool predictOrigin, CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version, float fractions[4], float normalzs[4])
	{
		...
		if (predictOrigin)
			postype = Move(player, vars, postype, wishspeed, traceFunc, pointContentsFunc, version, !strafed, a, fractions, normalzs);
	...
	PositionType PredictPast0msFrames(PlayerData& player, const MovementVars& vars, PositionType postype, const ProcessedFrame& out, const CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		if (curState.PredictThis == State0ms::DUCKED) {
			...
			postype = PredictDuck(player, vars, postype, curState_copy, out_copy, traceFunc, pointContentsFunc, version);
	...
	void CheckIfNextFrameShouldBe0ms(const PlayerData& player, const MovementVars& vars, const HLTAS::Frame& frame, PositionType postype, ProcessedFrame& out, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, const CurrentState& curState, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		if (frame.Lgagst || curState.LgagstsLeft) {
			...
			curStateCopy = CurrentState(curState);
			air.InDuckAnimation = true;
			postype = PredictDuck(air, vars, postype, curStateCopy, out_temp, traceFunc, pointContentsFunc, version);
	...
	ProcessedFrame MainFunc(const PlayerData& player, const MovementVars& vars, const HLTAS::Frame& frame, CurrentState& curState, const HLTAS::StrafeButtons& strafeButtons, bool useGivenButtons, TraceFunc traceFunc, PointContentsFunc pointContentsFunc, unsigned version)
	{
		...
		if (curState.ChangeTargetYawOffsetOver > 0) {
			float targetValue = static_cast<float>(NormalizeDeg(curState.ChangeTargetYawOffsetValue));
			float difference = static_cast<float>(GetAngleDifference(curState.TargetYawOffset, targetValue));
			...
			if (curState.ChangeTargetYawOffsetOver == 0)
				curState.TargetYawOffset = curState.ChangeTargetYawOffsetValue;
			else
				curState.TargetYawOffset += vars.Frametime * changeRate;
		}
		else {
			curState.TargetYawOffset = curState.ChangeTargetYawOffsetValue;
		}
		...
		auto postype = GetPositionType(playerCopy, traceFunc, pointContentsFunc, version);
		if (postype == PositionType::WATER) {
			VecCopy<float, 2>(player.Velocity, curState.LastVelocity);
			return out;
		}

		postype = PredictPast0msFrames(playerCopy, vars, postype, out, curState, traceFunc, pointContentsFunc, version);
		...
		postype = PredictDuck(playerCopy, vars, postype, curState, out, traceFunc, pointContentsFunc, version);
